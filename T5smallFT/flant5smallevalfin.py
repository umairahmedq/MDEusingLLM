...
for threshold in range(10, 110, 10):
    ...
    print(f"Rows with match percentage (flant5small with Updkw) >= {threshold}%: {count}")

# Save the updated DataFrame with the match percentage column back to the CSV file
df.to_csv("/home/jovyan/trainingmodel/keywordscourse/flant5small/test_keywords_with_match_percentages_upd.csv", index=False)
